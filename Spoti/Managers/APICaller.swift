...
final class APICaller {
    ...
    public func getFeaturedFlaylist(completion: @escaping ((Result<FeaturedPlaylistsResponse, Error>) -> Void)) {
        createRequest(
    ...
    public func search(with query: String, completion: @escaping(Result<[SearchResult],Error>)->Void){
        ...
        createRequest(with: URL(string: Constants.baseAPIURL+"/search?q=\(query.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "")"+"&type=album,track,playlist,artist&limit=7"), type: .GET) { request in
